...
*/
$get_data = mysqli_query( $conn, "SELECT RIGHT(id_operator,3) FROM operator ORDER BY RIGHT(id_operator,3) DESC" );
...
$custom_code = random_charo(3) . $get_3_number_of_year . '-'; // 7 karakter custom code dari sebelah kiri
...
*/
$kdo = ''; // Mendifinisikan nilai variable.
if ( $new_code >= 1 && $new_code < 10 ) :
  $kdo .= $custom_code."00".$new_code;
elseif ( $new_code >= 10 && $new_code < 100 ) :
  $kdo .= $custom_code."0".$new_code;
else :
  $kdo .= $custom_code.$new_code; // batas data di db hannya sampai 999 data
endif;
